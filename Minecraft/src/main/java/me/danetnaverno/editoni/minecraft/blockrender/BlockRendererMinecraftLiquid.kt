...
class BlockRendererMinecraftLiquid : BlockRenderer()
{
    ...
    {
        val height = (8 - (blockState as MinecraftLiquidState).level) / 8f - 0.1f

        GL11.glPushMatrix()
        GL11.glTranslatef(location.globalX.toFloat(), location.globalY.toFloat(), location.globalZ.toFloat())

        if (location.localY == 255/* || shouldRenderSideAgainst(block.chunk.getBlockAt(block.location.add(0, 1, 0)))*/)
        {
            texture.bind()
